...
function SidebarCollapseTrigger(props: ComponentProps<'button'>) {
  const { collapsed, setCollapsed } = Base.useSidebar();
  ...
  return (
    <button
      ...
      onClick={(event) => {
        props.onClick?.(event);
        if (!event.defaultPrevented) setCollapsed((previous) => !previous);
      }}
    />
  );
}
